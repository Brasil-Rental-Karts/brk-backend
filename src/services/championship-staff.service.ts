...
import {
  ...
  StaffPermissions,
  StaffRole,
} from '../models/championship-staff.entity';
...
export class ChampionshipStaffService extends BaseService<ChampionshipStaff> {
  ...
  async getStaffMembers(
    ...
    const staffMembers =
      await this.repository.findByChampionshipId(championshipId);
  ...
  async addStaffMember(
    ...
    const hasPermission = await this.hasChampionshipPermission(
      addedById,
      championshipId
    );
    if (!hasPermission) {
      throw new ForbiddenException(
        'Você não tem permissão para adicionar membros ao staff deste campeonato'
      );
    }
    ...
    if (userToAdd.id === championship.ownerId) {
      throw new BadRequestException(
        'O proprietário do campeonato já tem todas as permissões'
      );
    }

    // Verificar se já é membro do staff (ativo)
    const existingActiveStaff = await this.repository.findByUserAndChampionship(
      userToAdd.id,
      championshipId
    );
    if (existingActiveStaff) {
      throw new BadRequestException(
        'Este usuário já é membro do staff deste campeonato'
      );
    }
    ...
    if (existingStaff) {
      ...
      if (!existingStaff.isActive) {
        ...
        const reactivatedStaff = await this.repository.update(
          existingStaff.id,
          updateData
        );
        if (!reactivatedStaff) {
        ...
        const fullStaffMember = await this.repository.findByIdWithRelations(
          existingStaff.id
        );
        if (!fullStaffMember) {
  ...
  async removeStaffMember(
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        'Você não tem permissão para remover membros do staff deste campeonato'
      );
    }
  ...
  async getUserStaffChampionships(userId: string): Promise<string[]> {
    const staffChampionships =
      await this.repository.getUserStaffChampionships(userId);
    return staffChampionships.map(staff => staff.championshipId);
  }

  async hasChampionshipPermission(
    userId: string,
    championshipId: string
  ): Promise<boolean> {
  ...
  async hasSpecificPermission(
    ...
    const staffMember = await this.repository.findByUserAndChampionship(
      userId,
      championshipId
    );
  ...
  async updateStaffMemberPermissions(
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        'Você não tem permissão para gerenciar membros do staff deste campeonato'
      );
    }
    ...
    const updatedStaffMember = await this.repository.update(staffMemberId, {
      permissions,
    });
    ...
    // Carregar dados completos para retorno
    const fullStaffMember =
      await this.repository.findByIdWithRelations(staffMemberId);
    if (!fullStaffMember) {
      throw new Error('Erro ao carregar dados do membro do staff');
    }
