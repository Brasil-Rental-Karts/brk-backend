...
export class StageParticipationService {
   ...
   */
  private async validatePaymentForParticipation(
    ...
    if (
      ...
      if (hasOverduePayments) {
        throw new BadRequestException(
          'Não é possível confirmar participação pois há parcelas vencidas. Por favor, quite as parcelas em atraso para participar das etapas.'
        );
      }
    }
    ...
    if (season.hasPaymentCondition('por_temporada')) {
      ...
      if (
        ...
        throw new BadRequestException(
          'Para participar da etapa é necessário ter pelo menos um pagamento confirmado.'
        );
      }
    }
  }
  ...
  async confirmParticipation(
    ...
    const hasCategory = registration.categories.some(
      regCategory => regCategory.categoryId === categoryId
    );
    ...
    const stageCategoryIds = Array.isArray(stage.categoryIds)
      ? stage.categoryIds
      : stage.categoryIds
        ? (stage.categoryIds as string).split(',').map(id => id.trim())
        : [];

    if (!stageCategoryIds.includes(categoryId)) {
      throw new BadRequestException(
        'Esta categoria não está disponível nesta etapa'
      );
    }
   ...
   */
  async confirmParticipationAdmin(
    data: CreateParticipationData
  ): Promise<StageParticipation> {
    const { userId, stageId, categoryId } = data;
   ...
   */
  async hasUserConfirmedParticipation(
    userId: string,
    stageId: string
  ): Promise<boolean> {
   ...
   */
  async findUserUpcomingParticipations(
    userId: string
  ): Promise<StageParticipationWithDetails[]> {
   ...
   */
  async getAvailableCategoriesForUser(
    userId: string,
    stageId: string
  ): Promise<any[]> {
    // Buscar a etapa
