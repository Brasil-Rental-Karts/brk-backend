...
export class SeasonController extends BaseController {
  ...
  public initializeRoutes(): void {
    ...
    this.router.get(
      ...
      this.getSeasonsByChampionship.bind(this)
    );
    ...
    this.router.post(
      '/:id/refresh-cache',
      authMiddleware,
      this.refreshSeasonCache.bind(this)
    );
    this.router.post(
      '/:id/classification',
      authMiddleware,
      this.saveSeasonClassification.bind(this)
    );
  }
  ...
  private async getAllSeasons(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Erro ao buscar temporadas', details: error.message });
    }
  }

  private async saveSeasonClassification(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params; // seasonId
      const userId = req.user!.id;
      const payload = req.body; // JSON do frontend
      ...
      const hasPermission = await this.championshipStaffService.hasChampionshipPermission(
        userId,
        existingSeason.championshipId
      );
      if (!hasPermission) {
        res.status(403).json({
          message: 'Você não tem permissão para salvar classificação desta temporada.',
        });
        return;
      }
      ...
      res.status(200).json({ message: 'Classificação salva com sucesso' });
    } catch (error: any) {
  ...
  private async getSeasonsByChampionship(
    req: Request,
    res: Response
  ): Promise<void> {
    try {
      ...
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 10;
      const seasons = await this.seasonService.findByChampionshipId(
        championshipId,
        page,
        limit
      );
      ...
      res.status(500).json({
        message: 'Erro ao buscar temporadas do campeonato',
        details: error.message,
      });
    }
  }
  ...
  private async getSeasonById(req: Request, res: Response): Promise<void> {
    try {
      ...
      res
        .status(500)
        .json({ message: 'Erro ao buscar temporada', details: error.message });
    }
  }
  ...
  private async createSeason(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!hasPermission) {
        res.status(403).json({
          message:
            'Você não tem permissão para criar temporadas neste campeonato.',
          details:
            'Apenas administradores do sistema ou staff do campeonato podem realizar esta ação.',
        });
        return;
      }
      ...
      if (registrationOpen) {
        const championship = await this.championshipService.findById(
          req.body.championshipId
        );
        if (!championship) {
      ...
      if (
        ...
        Array.isArray(req.body.paymentConditions)
      ) {
        paymentConditions = req.body.paymentConditions.map(
          (condition: any) => ({
            type: condition.type,
      ...
      if (error instanceof BadRequestException) {
        ...
        res
          .status(500)
          .json({ message: 'Erro ao criar temporada', details: error.message });
      }
    }
  }
  ...
  private async updateSeason(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Verificar se está tentando abrir as inscrições e se o Wallet ID está configurado
      if (
        req.body.registrationOpen === true &&
        !existingSeason.registrationOpen
      ) {
        const championship = await this.championshipService.findById(
          existingSeason.championshipId
        );
      ...
      if (req.body.startDate)
        seasonData.startDate = new Date(req.body.startDate);
      ...
      if (req.body.registrationOpen !== undefined)
        seasonData.registrationOpen = req.body.registrationOpen;
      if (req.body.regulationsEnabled !== undefined)
        seasonData.regulationsEnabled = req.body.regulationsEnabled;
      ...
      if (
        ...
        seasonData.paymentConditions = req.body.paymentConditions.map(
          (condition: any) => ({
            type: condition.type,
            value: parseFloat(condition.value),
            description: condition.description || '',
      ...
      if (error instanceof NotFoundException) {
        ...
        res.status(500).json({
          message: 'Erro ao atualizar temporada',
          details: error.message,
        });
      }
    }
  ...
  private async deleteSeason(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (error instanceof NotFoundException) {
        ...
        res.status(500).json({
          message: 'Erro ao deletar temporada',
          details: error.message,
        });
      }
    }
  ...
  private async refreshSeasonCache(req: Request, res: Response): Promise<void> {
    try {
      ...
      const success = await this.seasonService.refreshSeasonCache(
        existingSeason.id
      );
      if (!success) {
        res
          .status(500)
          .json({ message: 'Erro ao atualizar cache da temporada' });
        return;
      }

      res
        .status(200)
        .json({ message: 'Cache da temporada atualizado com sucesso' });
    } catch (error: any) {
      if (error instanceof NotFoundException) {
        res.status(404).json({ message: error.message });
      } else {
        res.status(500).json({
          message: 'Erro ao atualizar cache da temporada',
          details: error.message,
        });
      }
    }
  ...
  private validateSeasonData(data: any, isCreate: boolean = true): void {
    ...
    if (
      data.description &&
      (data.description.length < 10 || data.description.length > 1000)
    ) {
      throw new BadRequestException(
        'Descrição deve ter entre 10 e 1000 caracteres'
      );
    }
    ...
    if (data.paymentConditions && Array.isArray(data.paymentConditions)) {
      for (const condition of data.paymentConditions) {
        if (
          !condition.type ||
          !['por_temporada', 'por_etapa'].includes(condition.type)
        ) {
          throw new BadRequestException(
            'Tipo de condição de pagamento inválido'
          );
        }

        if (typeof condition.value !== 'number' || condition.value <= 0) {
          throw new BadRequestException(
            'Valor da condição de pagamento deve ser um número positivo'
          );
        }

        if (
          condition.paymentMethods &&
          Array.isArray(condition.paymentMethods)
        ) {
        ...
        if (
          condition.pixInstallments !== undefined &&
          (typeof condition.pixInstallments !== 'number' ||
            condition.pixInstallments < 1 ||
            condition.pixInstallments > 12)
        ) {
          throw new BadRequestException(
            'Parcelamento PIX deve ser entre 1 e 12'
          );
        }

        if (
          condition.creditCardInstallments !== undefined &&
          (typeof condition.creditCardInstallments !== 'number' ||
            condition.creditCardInstallments < 1 ||
            condition.creditCardInstallments > 12)
        ) {
          throw new BadRequestException(
            'Parcelamento cartão de crédito deve ser entre 1 e 12'
          );
        }
      }
    }
