...
export class LapTimesController extends BaseController {
  ...
  public initializeRoutes(): void {
    ...
    this.router.get('/stage/:stageId/category/:categoryId', this.getLapTimesByStageAndCategory.bind(this));
    
    // GET /lap-times/stage/:stageId/category/:categoryId/user/:userId
    this.router.get('/stage/:stageId/category/:categoryId/user/:userId', this.getLapTimesByUser.bind(this));
    
    // POST /lap-times/stage/:stageId/category/:categoryId/user/:userId
    this.router.post('/stage/:stageId/category/:categoryId/user/:userId', this.saveLapTimes.bind(this));
    
    // POST /lap-times/stage/:stageId/category/:categoryId/import
    this.router.post('/stage/:stageId/category/:categoryId/import', this.importLapTimesFromExcel.bind(this));
  ...
  private async getLapTimesByStageAndCategory(req: Request, res: Response): Promise<void> {
    try {
      const { stageId, categoryId } = req.params;
      
      const lapTimes = await this.lapTimesService.getLapTimesByStageAndCategory(stageId, categoryId);
      
      res.status(200).json(lapTimes);
    } catch (error) {
      console.error('Erro ao buscar tempos volta a volta por etapa e categoria:', error);
      res.status(500).json({ message: 'Erro interno do servidor' });
    }
  }
  ...
  private async importLapTimesFromExcel(req: Request, res: Response): Promise<void> {
    try {
      const { stageId, categoryId } = req.params;
