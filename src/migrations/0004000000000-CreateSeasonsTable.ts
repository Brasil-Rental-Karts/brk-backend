...
export class CreateSeasonsTable0004000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."Seasons_status_enum" AS ENUM('agendado', 'em_andamento', 'cancelado', 'finalizado')`
    );

    // Create InscriptionType enum
    await queryRunner.query(
      `CREATE TYPE "public"."Seasons_inscriptiontype_enum" AS ENUM('mensal', 'anual', 'semestral', 'trimestral')`
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TRIGGER IF EXISTS seasons_notify_trigger ON "Seasons"`
    );
    ...
    // Drop the enums
    await queryRunner.query(
      `DROP TYPE "public"."Seasons_inscriptiontype_enum"`
    );
