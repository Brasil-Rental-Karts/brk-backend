...
export class UpdatePenaltyStatuses1769000000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const hasNotApplied = enumValues.some(
      (row: any) => row.value === 'not_applied'
    );
    const hasPending = enumValues.some((row: any) => row.value === 'pending');
    const hasCancelled = enumValues.some(
      (row: any) => row.value === 'cancelled'
    );
    ...
    if (hasPending) {
      await queryRunner.query(
        `UPDATE "penalties" SET "status" = 'applied' WHERE "status" = 'pending'`
      );
    ...
    if (hasCancelled) {
      await queryRunner.query(
        `UPDATE "penalties" SET "status" = 'not_applied' WHERE "status" = 'cancelled'`
      );
    ...
    if (hasPending || hasCancelled) {
      ...
      await queryRunner.query(
        `ALTER TABLE "penalties" ALTER COLUMN "status" DROP DEFAULT`
      );
      ...
      await queryRunner.query(
        `ALTER TABLE "penalties" ALTER COLUMN "status" SET DEFAULT 'applied'`
      );

      console.log(
        '✅ Enum atualizado para conter apenas: applied, not_applied, appealed'
      );
    }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!hasNotApplied) {
      console.log(
        '✅ Não é possível reverter - enum já está no estado original'
      );
      return;
    }
    ...
    // Reverter dados
    await queryRunner.query(
      `UPDATE "penalties" SET "status" = 'pending' WHERE "status" = 'applied'`
    );
    await queryRunner.query(
      `UPDATE "penalties" SET "status" = 'cancelled' WHERE "status" = 'not_applied'`
    );
